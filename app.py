...
from stockmon import config as config_mod
...
def main() -> None:
    ...
    parser.add_argument("--interval", type=float, default=None, help="行情刷新间隔（秒）")
    parser.add_argument("--threshold", type=float, default=None, help="告警阈值（%）")
    parser.add_argument("--open", action="store_true", help="启动后自动打开浏览器")
    parser.add_argument("--debug", action="store_true")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    cfg = config_mod.load(overrides={"host": args.host, "port": args.port,
